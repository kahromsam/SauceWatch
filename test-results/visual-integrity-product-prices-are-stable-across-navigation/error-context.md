...
- Name: visual-integrity.spec.ts >> product prices are stable across navigation
- Location: tests\visual-integrity.spec.ts:13:5

# Error details

```
Error: Prices changed after visiting the cart
...
> 22 |   expect(pricesAfterCart, 'Prices changed after visiting the cart').toEqual(pricesBeforeCart);
  ...
  30 | });
  31 | 
  32 | test('core navigation chrome is visible on the inventory page', async ({ inventoryPage }) => {
  33 |   // The cart icon and burger menu must always be present — their absence breaks navigation
  34 |   await expect(inventoryPage.cartIcon).toBeVisible();
  35 |   await expect(inventoryPage.burgerMenuButton).toBeVisible();
  36 | });
  37 | 
```